 ...
 * Baseline story pool.
 *
 * Keeps the app demoable when every live source fails, and gives a fresh
 * deployment something to serve on its very first request. The first three
 * mirror the stories shown in the Figma brief screen.
 *
 * Entries are [category, source, title, summary].
 */
export const SEED_STORIES: readonly (readonly [string, string, string, string])[] = [
...
/**
 * Seeded stories are deliberately backdated. Live headlines from RSS/GNews are
 * always newer, so they win the "most recent" ordering and the seed pool only
 * surfaces when a fetch returns nothing.
 */
export const SEED_AGE_MS = 4 * 24 * 60 * 60 * 1000;
